...
/// used to get fine-grained control over how asynchronous continuations are executed.
/// </para>
/// </remarks>
/// <example>
/// <para>
/// The following code will always succeed, and demonstrates that this class supports asynchronicity, reentrance, and
/// mutual exclusion.
/// </para>
/// <code>
...
{
    readonly AsyncLocal<object> _asyncLocalScope = new();
    static readonly Action<object?, CancellationToken> CancelTcs = (state, token) =>
    {
    ...
    TaskCompletionSource? _pending;
    readonly WorkQueue _queue = new();
    object? _owningScope;
    ...
    /// </para>
    /// <para>
    /// This is probably only useful for testing. The <see cref="SynchronizationContext"/> that is used internally is
    /// designed to serialize work that is posted to it. So if you're on that thread then you're on the only thread that
    /// can do work on that <see cref="SynchronizationContext"/>, and this <see cref="ReentrantAsyncLock"/> is able to
