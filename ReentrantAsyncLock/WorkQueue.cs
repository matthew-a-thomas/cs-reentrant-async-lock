namespace ReentrantAsyncLock
{
    ...
    sealed class WorkQueue : SynchronizationContext
    {
        ...
        readonly Queue<Entry> _entries = new Queue<Entry>();
        readonly object _gate = new object();
        bool _isPumping;
        static readonly Action<object?> PumpDelegate;
        static readonly SendOrPostCallback SetManualResetEventSlimDelegate;
        static readonly ConcurrentBag<ManualResetEventSlim> UnusedManualResetEvents = new ConcurrentBag<ManualResetEventSlim>();
        ...
        readonly struct Entry
        {
            ...
            public Entry(SendOrPostCallback callback, object? state, ExecutionContext? executionContext)
            {
                Callback = callback;
                State = state;
